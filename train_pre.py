...
Usage:
    ...
    $ python -m torch.distributed.launch --nproc_per_node 3 train.py --weights '' --cfg yolov5n_copy.yaml --data data/coco.yaml,data/VOC.yaml --epochs 500 --batch-size 192 --device 0,1,2 --hyp data/hyps/hyp.scratch-low_copy.yaml --name distill_global_VOC_notpre_0729 --clip --is_global
"""
...
def train(hyp, opt, device, callbacks):  # hyp is path/to/hyp.yaml or hyp dictionary
    ...
    for i in range(len(data_lis)):
        ...
        with open(data, encoding='ascii', errors='ignore') as f:
            ...
            data_dic_lis.append(data_dict)
    
    with torch_distributed_zero_first(LOCAL_RANK):
        # data_dict = data_dict or check_dataset(data)  # check if None
        for data_dict in data_dic_lis:
            check_dataset(data_dict)  # check
    train_path_lis=[]
    val_path_lis=[]
    for data_dict in data_dic_lis:
        train_path_lis.append(data_dict['train'])
        val_path_lis.append(data_dict['val'])
    # train_path, val_path = data_dict['train'], data_dict['val']
    val_path = val_path_lis[0]
    data_dict = data_dic_lis[0]
    ...
    if opt.is_global:
        ...
        preprocess = Compose([
            Resize((244, 244), interpolation=BICUBIC),
            CenterCrop((244, 244)),
    ...
    if pretrained:
        ...
        LOGGER.info(f'premodel len is {len(csd)}')
        if not resume and opt.clip:
    ...
    hyp['weight_decay'] *= batch_size * accumulate / nbs / WORLD_SIZE  # scale weight_decay
    ...
    if pretrained and not resume:
        scheduler = lr_scheduler.OneCycleLR(optimizer, max_lr=hyp['lr0'], total_steps=epochs, anneal_strategy='linear')
    else:
    ...
    # Trainloader
    train_loader, dataset = create_dataloader(train_path_lis,
                                              imgsz,
        ...
        for f in last, best:
            if f.exists():
                ...
                if f is best:
                    ...
                    if is_coco:
                        callbacks.run('on_fit_epoch_end', list(mloss) + list(mdloss) + list(results) + lr, epoch, best_fitness, fi)
...
def parse_opt(known=False):
    ...
    parser.add_argument('--clip', action='store_true', help='use clip for classifier distillation model')
    parser.add_argument('--is_global', action='store_true', help='use global for classifier distillation strategy')
