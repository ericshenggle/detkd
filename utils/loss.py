...
class ComputeLoss:
    ...
    def __init__(self, model, autobalance=False):
        ...
        self.BCEclassifier = BCEclassifier
    ...
    def __call__(self, p, targets, class_distill=False, s_feat=None, t_feat_label=None, is_coco=None):  # predictions, targets
        ...
        lobj *= self.hyp['obj']
        lcls *= self.hyp['cls']
        lclassifier *= self.hyp['dist']
        bs = tobj.shape[0]  # batch size

        return ((lbox + lobj + lcls + lclassifier) * bs, torch.cat((lbox, lobj, lcls, lclassifier)).detach()) if class_distill else \
                ((lbox + lobj + lcls) * bs, torch.cat((lbox, lobj, lcls)).detach())

    def build_targets(self, p, targets):
        # Build targets for compute_loss(), input targets(image,class,x,y,w,h)
        na, nt = self.na, targets.shape[0]  # number of anchors, targets
        tcls, tbox, indices, anch, indices_, filter_ = [], [], [], [], [], []
        ...
        return tcls, tbox, indices, anch, indices_, filter_
